...
app.get('/api/health', (req, res) => {
    res.json({ status: 'OK', message: 'GAZCOM API is running', timestamp: new Date().toISOString() });
});
...
app.get('/api/categories', async (req, res) => {
    try {
        ...
        else res.json(sampleCategories);
    } catch (error) {
...
app.get('/api/products/featured', async (req, res) => {
    try {
        ...
        if (result.rows.length > 0) res.json(result.rows);
        else res.json(sampleProducts.filter(p => p.is_featured));
    } catch (error) {
        console.error('Error fetching featured products:', error.message);
        res.json(sampleProducts.filter(p => p.is_featured));
    }
});
...
app.get('/api/products/category/:categoryId', async (req, res) => {
    ...
    try {
        ...
        if (result.rows.length > 0) res.json(result.rows);
        else res.json(sampleProducts.filter(p => p.category_id == categoryId));
    } catch (error) {
        console.error('Error fetching products by category:', error.message);
        res.json(sampleProducts.filter(p => p.category_id == categoryId));
    }
});
...
app.get('/api/products/category/:categoryId/count', async (req, res) => {
    const { categoryId } = req.params;
    try {
        const result = await pool.query('SELECT COUNT(*) FROM products WHERE category_id = $1', [categoryId]);
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            `INSERT INTO users (username, email, password_hash, full_name, phone, role) VALUES ($1, $2, $3, $4, $5, 'user') RETURNING id, username, email, full_name, phone, role`,
            [username, email, hashedPassword, full_name, phone]
        );
        const token = jwt.sign({ userId: result.rows[0].id, username: result.rows[0].username, role: 'user' }, process.env.JWT_SECRET || 'gazcom_secret_key', { expiresIn: '7d' });
        emailService.sendWelcomeEmail(result.rows[0]).catch(console.error);
        res.json({ token, user: { ...result.rows[0], role: 'user' } });
    } catch (error) {
...
app.post('/api/auth/login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ userId: user.id, username: user.username, role: user.role }, process.env.JWT_SECRET || 'gazcom_secret_key', { expiresIn: '7d' });
        res.json({ token, user: { id: user.id, username: user.username, email: user.email, full_name: user.full_name, phone: user.phone, role: user.role } });
    } catch (error) {
...
app.get('/api/auth/verify', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query('SELECT id, username, email, full_name, phone, role FROM users WHERE id = $1', [req.user.userId]);
...
app.put('/api/auth/update-profile', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(
            `UPDATE users SET full_name = $1, email = $2, phone = $3 WHERE id = $4 RETURNING id, username, email, full_name, phone, role`,
...
app.post('/api/auth/forgot-password', async (req, res) => {
    ...
    try {
        ...
        if (user.rows.length === 0) return res.json({ message: 'If an account exists, a reset link has been sent.' });
        const resetToken = crypto.randomBytes(32).toString('hex');
        const expiresAt = new Date(Date.now() + 3600000);
        await pool.query('INSERT INTO password_resets (user_id, token, expires_at) VALUES ($1, $2, $3)', [user.rows[0].id, resetToken, expiresAt]);
...
app.post('/api/auth/reset-password', async (req, res) => {
    ...
    try {
        const resetRecord = await pool.query('SELECT * FROM password_resets WHERE token = $1 AND expires_at > NOW()', [token]);
        if (resetRecord.rows.length === 0) return res.status(400).json({ error: 'Invalid or expired token' });
        const hashedPassword = await bcrypt.hash(password, 10);
...
app.get('/api/orders', authenticateToken, async (req, res) => {
    try {
        const result = await pool.query(
            `SELECT o.*, (SELECT COUNT(*) FROM order_items WHERE order_id = o.id) as items_count FROM orders o WHERE o.user_id = $1 ORDER BY o.created_at DESC`,
            [req.user.userId]
        );
...
app.get('/api/orders/:id', authenticateToken, async (req, res) => {
    const { id } = req.params;
    try {
        const orderResult = await pool.query('SELECT * FROM orders WHERE id = $1 AND user_id = $2', [id, req.user.userId]);
        if (orderResult.rows.length === 0) return res.status(404).json({ error: 'Order not found' });
        const itemsResult = await pool.query(`SELECT oi.*, p.name, p.image_url FROM order_items oi JOIN products p ON oi.product_id = p.id WHERE oi.order_id = $1`, [id]);
...
app.post('/api/mpesa/stkpush', authenticateToken, async (req, res) => {
    ...
    try {
        const result = await mpesaService.initiateSTKPush(formattedPhone, amount, `GAZCOM-${orderId}`, 'Payment for order');
        res.json(result);
    } catch (error) {
...
app.post('/api/mpesa/callback', async (req, res) => {
    ...
    if (callbackData.Body?.stkCallback?.ResultCode === 0) {
        const receipt = callbackData.Body.stkCallback.CallbackMetadata?.Item?.find(i => i.Name === 'MpesaReceiptNumber')?.Value;
...
app.post('/api/upload/image', verifyAdmin, cloudinaryService.upload.single('image'), async (req, res) => {
    try {
        ...
        const result = await cloudinaryService.uploadImage(req.file.buffer, { folder: 'gazcom_products' });
...
app.get('/api/images/optimized/:publicId', (req, res) => {
    const url = cloudinaryService.getOptimizedImageUrl(req.params.publicId);
    res.json({ url });
});

app.delete('/api/images/:publicId', verifyAdmin, async (req, res) => {
    try {
        const result = await cloudinaryService.deleteImage(req.params.publicId);
...
app.post('/api/admin/products', verifyAdmin, async (req, res) => {
    ...
    try {
        const result = await pool.query(
            `INSERT INTO products (name, description, category_id, sub_category, sku, image_url, stock_quantity, is_featured) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *`,
...
app.get('/api/admin/orders', verifyAdmin, async (req, res) => {
    try {
        const result = await pool.query('SELECT o.*, u.username, u.email FROM orders o LEFT JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC');
        res.json(result.rows);
    } catch (error) {
        console.error('Error fetching orders:', error);
        res.status(500).json({ error: 'Database error' });
